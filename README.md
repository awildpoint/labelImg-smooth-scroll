...
|          | `Ctrl` + `Z`          | 撤销对标注框或文件的改动           | **[已修改]** |

## 功能添加：
### 1. 安全删除（Trash 备份）
在右侧文件列表中选中图片，按 `Delete` 键（或菜单 `Ctrl+Shift+D`），将弹出确认对话框，列出即将移动的图片及其对应的标签文件（`.xml` / `.txt` / `.json`）。确认后，文件将移入 `trash/` 备份目录而非永久删除。
- **trash 目录位置**：始终位于当前打开目录的上一级。例如打开 `dataset/images/`，trash 位于 `dataset/trash/`。
- **不依赖目录命名**：无论子目录叫 `images`、`manual_images`、`Aimg` 还是其他名称，trash 路径始终可预测。
- **同名文件防覆盖**：trash 中已存在同名文件时自动追加 `_1`、`_2` 后缀。
- 删除操作可通过 `Ctrl+Z` 撤销。
### 2. Ctrl+Z 统一撤销
支持撤销最近一步操作，不影响当前图片的保存状态。
| 操作 | 撤销效果 |
|------|---------|
| 新增标注框（画框） | 移除该框 |
| 删除标注框 | 恢复该框 |
...
| 复制标注框 | 移除副本 |
| 编辑标签名 | 恢复旧标签名 |
| 删除图片文件 | 从 trash 移回原位置 |
- 快捷键：`Ctrl+Z`，也可通过菜单「文件 → 撤销」触发。
- 撤销栈在切换图片时自动清空，避免跨图误操作。
- 撤销操作无需弹窗确认。
### 3. 文件列表 Delete 快捷键
在右侧「文件列表」Dock 中选中图片后，直接按键盘 `Delete` 键即可触发安全删除（含确认弹窗），无需使用菜单或 `Ctrl+Shift+D`。
